...
import src.models.modules.attentions as attentions
import src.models.modules.commons as commons
import src.models.modules.modules as modules
from src.models.modules.commons import get_padding, init_weights
...
class DurationDiscriminatorV1(nn.Module):  # vits2
    ...
    def __init__(
        ...
        # we use this to condition the discriminator on the speaker embedding
        if gin_channels != 0:
          self.cond = nn.Conv1d(gin_channels, in_channels, 1)
                ...
                for i in range(n_flows):
                    ...
                    self.flows.append(
                        MonoTransformerFlowLayer(
                            channels, hidden_channels, mean_only=mean_only
                        )
                    )
            ...
            for i in range(n_flows):
                self.flows.append(
                    modules.ResidualCouplingLayer(
                        ...
                        n_layers,
                        gin_channels=gin_channels,
                        mean_only=mean_only, # origin, True
                    )
                )
                self.flows.append(modules.Flip())
...
class SynthesizerTrn(nn.Module):
    ...
    def __init__(
        ...
        n_heads,
        n_layers,
        n_layers_q, # we use 12, default 16
        n_flows, # we use 8, default 4
        kernel_size,
        p_dropout,
        resblock,
        resblock_kernel_sizes,
        ...
        **kwargs,
        ...
        if self.use_spk_conditioned_encoder and self.n_speakers == 0:
            raise ValueError(
    ...
    def update_current_mas_noise_scale(self, global_step):
